...
3.5 LLM Verification (GPT-4.1)         <- MỚI (chỉ khi confidence < ngưỡng)
...
| `VERIFIER_MODEL` | `gpt-4.1` | Model verifier (env `VERIFIER_MODEL`). |
...
- `OPENAI_API_KEY` cần quyền gọi `gpt-4.1`.
- Verifier có thể đề xuất slug mới → guardrail vẫn xử lý qua semantic snap / pending, không upsert ẩu.
